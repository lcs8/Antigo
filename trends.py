...
def analyze_tweet_sentiment(tweet):
    ...
    sentimento_total = 0
    contador_sentimento = 0
    for i in palavras: 
        s = get_word_sentiment(i) 
        if has_sentiment(s): 
            sentimento_total += s 
            contador_sentimento += 1   
    if sentimento_total!=0:
       return sentimento_total/contador_sentimento
    else:
        return average
...
def find_center(polygons):
   ...
   #decomposição geometrica
    soma_latitude = 0
    soma_longitude = 0
    soma_area = 0
    
    for i in polygons:
        centro_centroide = find_centroid(i)
        soma_latitude+=centro_centroide[0] * centro_centroide[2]
        soma_longitude+=centro_centroide[1] * centro_centroide[2]
        soma_area+=centro_centroide[2]
    return make_position(soma_latitude/soma_area, soma_longitude/soma_area)
...
def find_closest_state(tweet, state_centers):
    ...
    estado_proximo = None    
    for estado, centro_estado in state_centers.items():
        if menor_distancia == None:
            menor_distancia =  geo_distance(centro_estado, tweet_location(tweet))
            estado_proximo = estado           
            continue
        else:
            distancia = geo_distance(centro_estado, tweet_location(tweet)) 
